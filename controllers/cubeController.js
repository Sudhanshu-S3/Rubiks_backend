...
const getCubeStates = async (req, res) => {
    try {
        ...
        let cubeStates = await CubeState.find(query);

        // Manually sort the results since our implementation doesn't support .sort()
        cubeStates = cubeStates.sort((a, b) => {
...
const solveCube = async (req, res) => {
    try {
        ...
        if (!solution.success) {
            // It might be good practice to remove the saved state if solving fails,
            // or handle this case based on application requirements.
            // For now, we'll leave it saved.
            return res.status(400).json({
                success: false,
                stateId: newCubeState._id, // Include stateId even on failure if state was saved
        ...
        return res.json({
            success: true,
            stateId: newCubeState._id, // Add stateId to the successful response
